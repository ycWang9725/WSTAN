...
if __name__ == '__main__':
    ...
    def network(sample):
        ...
        if len(loss_ks) > 0:
            ...
            sorted_times = None if model.training else get_proposal_results(prob_ks[0].squeeze(1), duration)
        else:
            # print(map_mask.shape)
            # print(pred_ks[0].shape)
            # print(map_gt.shape)
            loss_value, joint_prob = loss.bce_rescale_loss(pred_ks[0], map_mask.repeat(1, map_gt.shape[1], 1, 1), map_gt,
                                                  config.LOSS.PARAMS)
