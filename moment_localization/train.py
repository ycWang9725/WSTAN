...
if __name__ == '__main__':
    ...
    train_dataset = getattr(datasets, dataset_name)('train', unsup=config.DATASET.UNSUP)
...
#         print(config.LOSS.NAME)
        ...
        if config.LOSS.NAME == 'bce_rescale_loss':
            pred, map_mask, _ = model(textual_input, textual_mask, visual_input)
            loss_value, joint_prob = loss.bce_rescale_loss(pred, map_mask, map_gt, config.LOSS.PARAMS)
            sorted_times = None if model.training else get_proposal_results(joint_prob, duration)
        elif 'triplet_loss' in config.LOSS.NAME:
            merged_prediction, map_mask, pred_ks = model(textual_input, textual_mask, visual_input)
            # print('3', pred_ks[0].shape)
            neg_v_prediction, neg_v_map_mask, _ = model(textual_input, textual_mask, neg_visual_input)
            # print(~(textual_mask.squeeze().bool()))
            # print(~(neg_textual_mask.squeeze().bool()))
            neg_t_prediction, neg_t_map_mask,  _ = model(neg_textual_input, neg_textual_mask, visual_input)
            loss_value = loss.triplet_loss(merged_prediction, map_mask,
                                       neg_v_prediction, neg_v_map_mask,
                                       neg_t_prediction, neg_t_map_mask, config.LOSS.PARAMS)
            # del merged_prediction, neg_v_prediction, neg_v_map_mask, neg_t_prediction, neg_t_map_mask

            # self restrict
            loss_self_restrict, joint_prob = loss.self_restrict_loss(pred_ks[0], map_mask, config.LOSS.PARAMS)
            sorted_times = None if model.training else get_proposal_results(joint_prob, duration)
            # warming = False
            if 'self_restrict_loss' in config.LOSS.NAME:
                loss_value += loss_self_restrict
            ...
            if not warming:
                if len(pred_ks) > 1:
                    ...
                    loss_ks = []
                    prob_ks = []
                    for k in range(1, len(pred_ks)):
                        loss_k, joint_prob_k = loss.refinement_loss(pred_ks[k], map_mask, pred_ks[k-1],
                                                                   config.LOSS.PARAMS)
                        loss_ks.append(loss_k)
                        prob_ks.append(joint_prob_k)
                        if 'self_restrict_loss' in config.LOSS.NAME:
                            loss_ref_self_restrict, _ = loss.self_restrict_loss(pred_ks[k], map_mask,
                                                                                config.LOSS.PARAMS)
                            loss_value += loss_ref_self_restrict
                    # print(loss_value, [loss_k for loss_k in loss_ks])
                    loss_value += sum(loss_ks)
                    # sorted_times = None if model.training else get_proposal_results(torch.mean(torch.stack(prob_ks, 0), dim=0).squeeze(1), duration)
                    sorted_times = None if model.training else get_proposal_results(prob_ks[-1].squeeze(1), duration)
        else:
