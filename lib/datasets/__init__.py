...
#     print(type(max_len), type(trailing_dims), type(len(sequences)))
#     print(tuple(max_len), (len(sequences)), tuple(trailing_dims))
    if batch_first:
        out_dims = [len(sequences)] + max_len + list(trailing_dims)
    else:
        out_dims = max_len + [len(sequences)] + list(trailing_dims)
    
#     print(out_dims, type(out_dims))
...
def align_videos(batch_videos):
    # head_dims = batch_videos[0].size()[0]
    w, h = batch_videos[0].size()[-2:]
    avg_frame_num = config.DATASET.NUM_FRAMES
    # avg_frame_num = sum([v.size(0) for v in batch_videos]) // len(batch_videos)
    # print(avg_frame_num)
    out_dims = [avg_frame_num, w, h]
    new_batch_videos = []
    for i, v in enumerate(batch_videos):
        # print(v.shape, out_dims)
        tmp = torch.nn.functional.interpolate(v.transpose(0, 1).unsqueeze(0), size=out_dims, mode='trilinear', align_corners=True)
        # print(tmp.shape)
        tmp = tmp.squeeze(0).transpose(0, 1)
        # print(tmp.shape)
        new_batch_videos.append(tmp)

    return torch.stack(new_batch_videos)
...
def collate_fn(batch):
    ...
    max_num_sents = max([map_gt.shape[0] for map_gt in batch_map_gt])
    max_num_clips = max([map_gt.shape[-1] for map_gt in batch_map_gt])
    padded_batch_map_gt = torch.zeros(len(batch_map_gt), max_num_sents, max_num_clips, max_num_clips)
    for i, map_gt in enumerate(batch_map_gt):
        num_sents = map_gt.shape[0]
        num_clips = map_gt.shape[-1]
        padded_batch_map_gt[i][:num_sents,:num_clips,:num_clips] = map_gt
